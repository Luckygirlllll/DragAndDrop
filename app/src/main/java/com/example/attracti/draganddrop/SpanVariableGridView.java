...
public class SpanVariableGridView extends AdapterView<BaseAdapter> {
    ...
    private void initialize(final AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.CoolDragAndDropGridView);

            try {
                mColCount = a.getInteger(R.styleable.CoolDragAndDropGridView_numColumns, 2);
                mItemMargin = a.getDimensionPixelSize(R.styleable.CoolDragAndDropGridView_itemMargin, 0);
